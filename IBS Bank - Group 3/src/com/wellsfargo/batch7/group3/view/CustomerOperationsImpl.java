...
public class CustomerOperationsImpl implements ICustomerDao {
	...
	public FundsTransfer transferFunds(FundsTransfer fndsTrf) {
		...
		if(fndsTrf != null) {
			if ((fndsTrf.getCustAcctNum() == fndsTrf.getBnfcryAcctNum())) {
				fndsTrf.setTxnStatus("Transfer between same account is not possible");
			} else if (fndsTrf.getBnfcryTrnsfrAmt() < avlblBal) {
				fndsTrf.setTxnStatus("");
			} else {
